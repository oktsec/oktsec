...
var quarantineTmpl = template.Must(template.New("quarantine").Funcs(template.FuncMap{
...
<h1>Quarantine <span>Queue</span></h1>
<p class="page-desc">Messages held for human review before delivery. Approve or reject quarantined content.</p>

<div class="stats" style="grid-template-columns:repeat(4,1fr)">
  <div class="stat">
    <div class="label">Pending</div>
    <div class="value warn">{{.Stats.Pending}}</div>
  </div>
  <div class="stat">
    <div class="label">Approved</div>
    <div class="value success">{{.Stats.Approved}}</div>
  </div>
  <div class="stat">
    <div class="label">Rejected</div>
    <div class="value danger">{{.Stats.Rejected}}</div>
  </div>
  <div class="stat">
    <div class="label">Expired</div>
    <div class="value" style="color:var(--text3)">{{.Stats.Expired}}</div>
  </div>
</div>

<div class="tabs" data-tab-group="qstatus">
  <a href="/dashboard/quarantine?status=pending" class="tab {{if eq .StatusFilter "pending"}}active{{end}}">Pending</a>
  <a href="/dashboard/quarantine?status=approved" class="tab {{if eq .StatusFilter "approved"}}active{{end}}">Approved</a>
  <a href="/dashboard/quarantine?status=rejected" class="tab {{if eq .StatusFilter "rejected"}}active{{end}}">Rejected</a>
  <a href="/dashboard/quarantine?status=" class="tab {{if eq .StatusFilter ""}}active{{end}}">All</a>
</div>
...
<div class="card">
  ...
  <table>
    ...
    <tr id="q-row-{{.ID}}">
      ...
      <td>
        {{if eq .Status "pending"}}<span class="badge-pending">pending</span>
        {{else if eq .Status "approved"}}<span class="badge-approved">approved</span>
        {{else if eq .Status "rejected"}}<span class="badge-blocked">rejected</span>
        {{else if eq .Status "expired"}}<span class="badge-expired">expired</span>
        {{else}}{{.Status}}{{end}}
      </td>
      <td style="font-size:0.72rem">{{.ExpiresAt}}</td>
      <td>
        <div style="display:flex;gap:4px;align-items:center">
          ...
          <button class="btn btn-sm" style="padding:2px 8px;font-size:0.7rem;background:var(--success)" hx-post="/dashboard/api/quarantine/{{.ID}}/approve" hx-target="#q-row-{{.ID}}" hx-swap="outerHTML">approve</button>
          <button class="btn btn-sm btn-danger" style="padding:2px 8px;font-size:0.7rem" hx-post="/dashboard/api/quarantine/{{.ID}}/reject" hx-target="#q-row-{{.ID}}" hx-swap="outerHTML">reject</button>
          {{end}}
        </div>
      </td>
    </tr>
  ...
  {{else}}
  <div class="empty">No quarantine items matching this filter.</div>
  {{end}}
</div>
` + layoutFoot))
...
var analyticsTmpl = template.Must(template.New("analytics").Funcs(template.FuncMap{
	...
	"maxRuleCount": func(rules []audit.RuleStat) int {
		m := 0
		for _, r := range rules {
			if r.Count > m {
				m = r.Count
	...
	"riskPct": func(score float64) int {
		p := int(score)
		if p > 100 {
			p = 100
		}
...
<h1>Incident <span>Analytics</span></h1>
<p class="page-desc">Security incident trends, top triggered rules, and agent risk scoring.</p>

<div class="stats" style="grid-template-columns:repeat(4,1fr)">
  <div class="stat" style="border-left:3px solid var(--danger)">
    <div class="label">Critical</div>
    <div class="value danger">{{.CriticalCount}}</div>
  </div>
  <div class="stat" style="border-left:3px solid var(--warn)">
    <div class="label">High</div>
    <div class="value warn">{{.HighCount}}</div>
  </div>
  <div class="stat" style="border-left:3px solid var(--accent-light)">
    <div class="label">Medium</div>
    <div class="value" style="color:var(--accent-light)">{{.MediumCount}}</div>
  </div>
  <div class="stat" style="border-left:3px solid var(--text3)">
    <div class="label">Low</div>
    <div class="value" style="color:var(--text3)">{{.LowCount}}</div>
  </div>
</div>
...
<div class="card">
  ...
  {{if .TopRules}}
  {{$max := maxRuleCount .TopRules}}
  <table>
    <thead><tr><th>Rule</th><th>Severity</th><th>Count</th><th style="width:40%">Distribution</th></tr></thead>
    <tbody>
    {{range .TopRules}}
    <tr class="clickable" hx-get="/dashboard/api/rule/{{.RuleID}}" hx-target="#panel-content" hx-swap="innerHTML">
      <td style="font-weight:600">{{.RuleID}}<br><span style="font-weight:400;color:var(--text3);font-size:0.72rem">{{.Name}}</span></td>
      <td>
        {{if eq .Severity "critical"}}<span class="sev-critical">critical</span>
        {{else if eq .Severity "high"}}<span class="sev-high">high</span>
        {{else if eq .Severity "medium"}}<span class="sev-medium">medium</span>
      ...
      <td>
        <div class="risk-bar">
          <div class="risk-bar-fill {{if eq .Severity "critical"}}risk-high{{else if eq .Severity "high"}}risk-med{{else}}risk-low{{end}}" style="width:{{pct .Count $max}}%"></div>
        </div>
      </td>
    </tr>
    {{end}}
  ...
  {{else}}
  <div class="empty">No rules triggered yet.</div>
  {{end}}
</div>
...
<div class="card">
  ...
  {{if .AgentRisk}}
  <table>
    <thead><tr><th>Agent</th><th>Total</th><th>Blocked</th><th>Quarantined</th><th>Risk Score</th><th style="width:30%"></th></tr></thead>
    <tbody>
    {{range .AgentRisk}}
    <tr class="clickable" onclick="window.location='/dashboard/agents/{{.Agent}}'">
      <td style="font-weight:600">{{.Agent}}</td>
      <td>{{.Total}}</td>
      <td>{{if .Blocked}}<span class="badge-blocked">{{.Blocked}}</span>{{else}}0{{end}}</td>
      <td>{{if .Quarantined}}<span class="badge-quarantined">{{.Quarantined}}</span>{{else}}0{{end}}</td>
      <td>{{printf "%.1f" .RiskScore}}</td>
      <td>
        <div class="risk-bar">
          <div class="risk-bar-fill {{riskClass .RiskScore}}" style="width:{{riskPct .RiskScore}}%"></div>
        </div>
      </td>
    </tr>
    {{end}}
